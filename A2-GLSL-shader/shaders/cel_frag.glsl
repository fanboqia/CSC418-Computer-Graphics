...
void main() {
  ...
  float diffuse = dot(lightVec,normalInterp);	
  //why max function is not supported here....
  if(diffuse < 0.0)
    diffuse = 0.0;
  float levels = 4.0;
  float diffuse_level = floor(diffuse * levels);
  diffuse = diffuse_level / levels;
  ...
  vec3 final_color = Ka  * ambientColor + Kd * diffuse * diffuseColor + specular * specularColor;
