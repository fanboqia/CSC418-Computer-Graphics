...
//declare here because of compilation requirement
struct Texture;

struct Material {
	Material(Color ambient, Color diffuse, Color specular, double exp, Texture* texture, bool isGlossy) :
		ambient(ambient), diffuse(diffuse), specular(specular),
		specular_exp(exp),texture(texture), isGlossy(isGlossy){}
...
struct Ray3D {
	...
	//save texture uv coordinate
	double u_coord;
	double v_coord;
};

//newly added structure Texture to add on Material struct
struct Texture{
	...
	Texture(char *fileName){
		...
		bmp_read(fileName,&this->width,&this->height,&this->rarray,&this->garray,&this->barray);


		// The logic here is when you find error then return true, which is inverted...
		// if(bmp_read(fileName,&this->width,&this->height,&this->rarray,&this->garray,&this->barray)){
	...
	//convert the ray uv coordinate to picture position
	Color getColor(Ray3D &ray){
		unsigned u = ray.u_coord * this->width;
		unsigned v = ray.v_coord * this->height;

		//map index to one dimension array
		unsigned index = u*width+v;
		//convert from 0 to 255 rgb to 0 to 1 rgb 
		Color col = Color(rarray[index]/255.0,garray[index]/255.0,barray[index]/255.0);
		//change the intersection color for recursive ray to collect the texture color
